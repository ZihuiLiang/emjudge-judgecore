...
fn main() {
    ...
    let input  = vec![];
    let mut output = vec![];
    File::open("examples/programs/permission/eval.cpp").unwrap().read_to_end(&mut eval_script).unwrap();
    File::open("examples/programs/permission/main.cpp").unwrap().read_to_end(&mut test_script).unwrap();
    File::open("examples/programs/permission/out").unwrap().read_to_end(&mut output).unwrap();
