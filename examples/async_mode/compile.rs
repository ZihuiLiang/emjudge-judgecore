...
async fn main() {
    let mut script = vec![];
    tokio::fs::File::open("examples/programs/compile_error.cpp").await
        .unwrap()
        .read_to_end(&mut script).await
        .unwrap();
    println!("Compiling examples/programs/compile_error.cpp in language C++...");
    println!("{:?}", RawCode::new(script, String::from("C++")).compile().await);
    ...
    println!("{:?}", RawCode::new(script, String::from("D++")).compile().await);
}
